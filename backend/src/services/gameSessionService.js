...
const saveGameSession = async ({ studentId, lessonId, score, accuracy, timeSpentSec, difficultyLevel }) => {
  ...
  return prisma.$transaction(async (tx) => {
    const session = await tx.gameSession.create({ data: { studentId, lessonId, score, accuracy, timeSpentSec, difficultyLevel } });
...
const completeSession = async ({ studentId, sessionId, score, accuracy, timeSpentSec, completion = 1, hintsUsed = 0, mistakesCount = 0 }) => {
  ...
  const xpEarned = 10 + Math.round(safeAccuracy * 40) + (completion >= 1 ? 20 : Math.round(completion * 20)) + (mistakesCount === 0 && safeAccuracy >= 0.9 ? 10 : 0);
  const result = await prisma.$transaction(async (tx) => {
    const updated = await tx.gameSession.update({ where: { id: sessionId }, data: { score: Number(score || 0), accuracy: safeAccuracy, timeSpentSec: Math.max(0, Number(timeSpentSec || 0)), completion: Math.max(0, Math.min(1, Number(completion))), hintsUsed: Math.max(0, Number(hintsUsed || 0)), mistakesCount: Math.max(0, Number(mistakesCount || 0)), status: 'COMPLETED', xpEarned, completedAt: new Date() } });
    await tx.xPTransaction.create({ data: { studentId, sessionId, xpAmount: xpEarned, reason: 'GAME_COMPLETION', metadata: { score, accuracy: safeAccuracy, completion } } });
    return updated;
  });
  if (session.game) {
    await updateTopicMastery({ studentId, topicId: session.game.topicId, correctness: safeAccuracy >= 0.6, responseTimeSec: timeSpentSec, completed: true, hintUsed: hintsUsed > 0 });
    if (session.game.subtopicId) await updateSubtopicMastery({ studentId, subtopicId: session.game.subtopicId, correctness: safeAccuracy >= 0.6, responseTimeSec: timeSpentSec, completed: true, hintUsed: hintsUsed > 0 });
  }
...
const skipSession = async ({ studentId, sessionId, timeSpentSec = 0 }) => {
  const session = await prisma.gameSession.findFirst({ where: { id: sessionId, studentId }, include: { game: true } });
  if (!session) { const err = new Error('Session not found.'); err.statusCode = 404; throw err; }
  if (session.status !== 'STARTED') return session;
...
const abandonSession = async ({ studentId, sessionId, timeSpentSec = 0, completion = 0 }) => prisma.gameSession.updateMany({ where: { id: sessionId, studentId, status: 'STARTED' }, data: { status: 'ABANDONED', timeSpentSec: Math.max(0, Number(timeSpentSec)), completion: Math.max(0, Math.min(1, Number(completion))), completedAt: new Date() } });
